...
export default function PaywallScreen() {
  ...
  useEffect(() => {
    trackPaywallView('upgrade_button'); // Default trigger
  }, []);

  const handleSubscribe = async () => {
    const productId = selectedPlan === 'monthly' ? 'prod_monthly' : 'prod_weekly';
    
    setIsLoading(true);
    trackSubscribeAttempt(productId);
    
    try {
      await purchase(productId);
      trackPaywallClose('purchase');
        ...
        >
          {isLoading ? (
            ...
            <Text style={styles.subscribeButtonText}>
              Unlock Pro — ${selectedPlan === 'monthly' ? '9.99/mo' : '2.99/wk'}
            </Text>
          )}
        </TouchableOpacity>
