...
export default function AnalyzeScreen() {
  ...
  const { analyzeConversation, isAnalyzing, checkGating, isGated } = useAnalysisStore();
  ...
  const handleAnalyze = async () => {
    ...
    // Check if analysis is gated before proceeding
    const gatingCheck = checkGating();
    if (!gatingCheck.canAnalyze) {
      Alert.alert(
        'Daily Limit Reached',
        gatingCheck.reason || 'Upgrade to Pro for unlimited analyses.',
        [
    ...
    try {
      ...
      // Check if error is due to gating
      if (isGated) {
        Alert.alert(
          'Daily Limit Reached',
